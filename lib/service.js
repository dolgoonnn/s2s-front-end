...
export function postReview(id, data) {
    return axiosClient
        .post(`/api/teaching/matching/${id}/rating`, { ...data })
        .then((response) => {
            return response;
        });
}
...
export function getReview(id) {
    return axiosClient.get(`/api/teaching/${id}/rating`).then((response) => {
        return response.data;
    });
}

export function useReview(id) {
    const { data, error } = useSWR(`/api/teaching/${id}/rating`, async () =>
        getReview(id)
    );
